...
<AuthCheck>
  ...
  {:else}
    <form class="w-full" onsubmit={confirmUsername}>
      <input
        type="text"
      ...
      />
      <div class="my-4 min-h-16 px-8 w-full flex justify-center">
        {#if loading}
        ...
        {#if isAvailable}
          <button class="btn w-4/5 btn-success">Confirm username @{username} </button>
        {/if}
      </div>
    </form>
  {/if}
